...
wget ${NODEBUG} --no-cache "${REPOSITORY_URL}${REPO_BRANCH}/mister-clone.sh" -O /tmp/mister-clone.sh
check4error "${?}"
cmp -s /tmp/mister-clone.sh /media/fat/Scripts/mister-clone.sh
if [ "${?}" -gt "0" ] && [ -s /tmp/mister-clone.sh ]; then
    echo -e "${fyellow}Script Updated ${fmagenta}${PICNAME}${freset}"
    mv -f /tmp/mister-clone.sh /media/fat/Scripts/mister-clone.sh
    exec /media/fat/Scripts/mister-clone.sh
    exit 255
else
    rm /tmp/mister-clone.sh
fi
